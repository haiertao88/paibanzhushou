...
# 生成完整的网页版代码 - 海风米奇奇迹版 Web 移植
# 采用 Streamlit，支持部署到 Streamlit Cloud
...
功能覆盖：AI文案生成、双语翻译、排版优化、图库管理、背景注入、微调工具
...
def save_config_to_file():
    cfg = {k: st.session_state.get(k, v) for k, v in load_config().items()}
    try:
...
with st.sidebar:
    ...
    with st.expander("🖼️ 背景图路径", expanded=False):
        st.text_input("标准封面路径", key="std_cover_path")
        st.text_input("标准正文路径", key="std_body_path")
        st.text_input("中性封面路径", key="neu_cover_path")
        st.text_input("中性正文路径", key="neu_body_path")
...
print("✅ 代码已生成，长度:", len(code), "字符")
print("📁 文件保存至: /mnt/agents/output/app.py")
